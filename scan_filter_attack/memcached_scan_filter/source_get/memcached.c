...
void setup_udp_header(struct udphdr *udph)
{
    ...
    udph->check = 0;
    char res[100] = "\x00\x01\x00\x00\x00\x01\x00\x00";
    sprintf(res + 8, "get %s\r\n", tmp_md5);
    fprintf(stderr, "Package content: %s\n", res + 8);
    memcpy((void *)udph + sizeof(struct udphdr), res, 46);
...
int main(int argc, char *argv[ ])
{
    ...
    Compute_string_md5((unsigned char *) argv[1], strlen(argv[1]), tmp_md5);
    srand(time(NULL));
    int i = 0;
    head = NULL;
